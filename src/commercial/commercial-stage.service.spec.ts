  ...
  return {
    lead: {
      findFirstOrThrow: jest
        .fn()
        .mockResolvedValue({ id: 'lead-1', tenantId: 't1', currentStage: stage, score: 5 }),
...
describe('CommercialStageService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [CommercialStageService, { provide: AuditLogService, useValue: mockAuditLog }],
    }).compile();
  ...
  it('should NOT transition from LEAD on unknown signal', async () => {
    ...
    const result = await service.processSignal(
      ...
      'lead-1',
      'conv-1',
      'confirmacion_de_pago_pendiente' as any,
    );
