...
# Uso: CLAUDE_API_KEY=sk-ant-... bash demo/start.sh
# ============================================================
...
CLAUDE_KEY="${CLAUDE_API_KEY:-${1:-}}"

if [ -z "$CLAUDE_KEY" ]; then
  echo -e "${RED}Error: necesitas tu API key de Claude.${NC}"
  echo "  Uso: CLAUDE_API_KEY=sk-ant-... bash demo/start.sh"
  exit 1
fi
...
cd demo
CLAUDE_API_KEY="$CLAUDE_KEY" docker compose up -d --build postgres redis orchestrator gui
...
for i in $(seq 1 30); do
  ...
  if [ "$i" -eq 30 ]; then
    echo -e "${RED}  ✗ Timeout esperando al orquestador. Revisa: docker logs saas-demo-orchestrator${NC}"
    exit 1
  fi
  sleep 2
done
...
CLAUDE_API_KEY="$CLAUDE_KEY" node demo/seed/seed.js
...
echo ""
echo -e "  Para agregar WhatsApp real:"
